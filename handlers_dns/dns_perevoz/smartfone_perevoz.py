...
async def smart_per(message: types.Message):
        ...
        url = f"https://www.dns-shop.ru/catalog/actions/?stock=now&category=17a8a01d16404e77&action=tovarysoskidkoj"
        driver.get(url)
        ...
        for i in range(int(page)):
                ...
                url = f"https://www.dns-shop.ru/catalog/actions/?stock=now&category=17a8a01d16404e77&action=tovarysoskidkoj&p={i}"
                driver.get(url)
...
def register_handlers_smart_per(dp: Dispatcher):
    dp.register_message_handler(smart_per, commands=['Смартфоны_Перевоз'])
